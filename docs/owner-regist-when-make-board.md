...
## 選択肢・判断が必要な点（ユーザー選択結果）

1. **複数プロファイル管理UI**
   - ✅ Option A: プロファイル選択画面を最初に表示（ダッシュボード化）

2. **既存データの マイグレーション**
   - ✅ Option C: 新規データのみ Supabase に保存、既存は無視

3. **Cloudflare Workers との連携** ⚠️ 要確認
   - 現在: Google OAuth は Cloudflare Workers 経由
   - 問題: Supabase Auth と二重認証になる可能性
   - 未決定: トークン連携設定が必要か
